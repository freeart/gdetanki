						...
						return function (e, element, lastArg, steps) {
							...
							for (var s = 0; s < actions.length; s++) {
								...
								if ($.isFunction(check_fn)) {
									var res = check_fn.call(actionConfig, e, element, lastArg, steps);
									if (res && res.hasOwnProperty('promise')) {
										local.push(res);
									} else {
										var df = $.Deferred();
										df.resolve(res);
							...
							$.when.apply($, local).then(function () {
								...
								for (var p = 0; p < args.length; p++) {
									if (args[p] === undefined) nulls++;
								}
								lastArg = args.length && nulls < args.length ? args : lastArg;

								results.resolve(lastArg);
							});
					...
					!function (fn, step, name) {
						actions.push(function () {
							...
							try {
								...
								if (defaults.debugOnStep == name) debugger;
							...
							if ($.isPlainObject(lastArg) && $.isPlainObject(localResult)) {
								localResult = $.extend(true, {}, lastArg, localResult);
							}
							return localResult;
						});
					}(fn, i, rawConfig[i]);
							...
							if ($.isFunction(fn)) {
								!function (fn, step, name) {
									actions.push(function () {
										...
										if ($.isPlainObject(lastArg) && $.isPlainObject(localResult) && !localResult.promise) {
											localResult = $.extend(true, {}, lastArg, localResult);
										}
										return localResult;
									});
